...
class Picture:
    ...
    def processImage(self):
        image = read_image(self.original_path, cv2.IMREAD_COLOR)
        ...
        ##############################################################
        mask_path = self.fovmask_path  # TODO adresowanie tego ładnie jak inne
...
def getPaths():
    ...
    for i in range(len(healthy_paths)):
        pictures.append(Picture(healthy_paths[i], healthy_fovmask_paths[i], healthy_manual_paths[i]))
    return pictures
...
paths = getPaths()
test_image = paths[0].processImage()
